# &lt;blog&gt;-editor — Wisp editor (Hermes profile distribution)

A dedicated Hermes agent that researches, writes, SEO/GEO-optimizes, and publishes
articles to a Wisp blog. It is **separate from the blog content repo** — its only
...
- Skills auto-register as slash commands, e.g. `/wisp-publish`, `/ai-seo`, `/schema-markup`.
- Calls the Wisp MCP tools (`create_draft`, `validate_post`, `publish_post`, `get_analytics`, …).
...
hermes profile create <blog>-editor \
  --description "Researches, writes, SEO/GEO-optimizes and publishes articles to the Wisp MDX repo."
# then copy SOUL.md + config.yaml into ~/.hermes/profiles/<blog>-editor/
# and run the Wisp skill resolver to sync the skill set.
```
